...
| **Регенерація** | Повтор `POST` без прапора — **безкоштовний і не викликає AI**. `{ "regenerate": true }` — **платно**, ті самі credits кроку. **Максимум 3 спроби на крок у проєкті** (1 Make + 2 retry). Credits списуються **до** виклику провайдера; якщо AI вже пішов — credits не повертаються. | Інакше користувач крутить DALL·E/TTS за наш рахунок, доки є баланс. |
...
8. Якщо голос або кадр не сподобався — **Regenerate** цього кроку за ті самі credits (§7.2), не обов’язково новий проєкт. Звичайний повтор кнопки Make на вже зробленому кроці credits **і AI не чіпає**. На крок — **не більше 2 повторів** (разом 3 спроби на Reel): кожен реальний виклик AI ми оплачуємо вендору.
...
- **ліміт спроб:** `MAX_STEP_ATTEMPTS = 3` на `(project, step)` (succeeded + running + failed). Далі 429, AI не викликається;
...
| `POST /steps/visuals` з `{ "regenerate": true, "sceneId": N }` | **8** | лише цей кадр; mp4 скидається; лічить у ліміт Visuals |
| Спроба > 3 на цей крок у цьому проєкті | 429 | AI не викликати. Текст: *This step has been generated enough times…* |
...
Кнопка в панелі: *Not this {step}? Try again · N credits*. Після 2 повторів кнопка disabled.

Ліміт **на проєкт і крок**, не на акаунт: новий Reel = нові 3 спроби. Credits самі по собі не захищають маржу, поки ціни FROZEN.
...
5. Повторний Idea **без** `regenerate` не списує 5 credits і **не викликає AI**; **з** `regenerate` — списує і каскадить; 4-та спроба Idea на тому ж проєкті — 429 (§7.2).
...
| Regenerate | платний повтор кроку, макс. 2 на крок/проєкт; ідемпотентний POST без прапора — безкоштовний і без AI |
