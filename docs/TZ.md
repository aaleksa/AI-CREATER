...
`poster.ts` (`composeInvitePoster` тощо) у репо є, пайплайн його **не** кличе. Не підключати для тексту постера. **Виняток лише для лого:** якщо `logo_on_photos=1`, після живого кадру **ffmpeg** накладає PNG/JPG власника в кут. Це не compose постера і не «намалюй лого» в OpenAI.
...
| logo_on_photos | int 0/1 | чи накласти **файл** лого на кадр після генерації (ffmpeg). Дефолт 0. Не просити модель намалювати знак |
...
Перед `failed` на єдиному кадрі: **один додатковий авто-retry** `images.generate` (не лише 5xx). Якщо й він порожній — тоді `failed`.
...
Brand Kit, коли `use_brand=1`: кольори / ніша / лого — у **тексті** промпта. Зал / людина / товар — ще й **файли** в `images.edit`. Лого на фото — лише шар після кадру, не referenceImages. Користувач вендора не обирає.
...
`images.edit` з 1–3 референс-фото майже напевно дорожчий за чистий `images.generate` (більше вхідних токенів на зображення). Поточні FROZEN-цифри Visuals цього **не** включають. На беті писати в `actual_cost_gbp`, чи був edit, і скільки рефів. Overlay лого — локальний ffmpeg, **£0**.
